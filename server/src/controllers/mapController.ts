...
export const mapController = {
  ...
  async update(request: Request, response: Response) {
    ...
    const map = await runStoreQuery(async (database) => await readMapEditorMap(database, campaignId, mapId), {
      queueKey: `campaign:${campaignId}`
    });
    if (!map) {
      throw new HttpError(404, "Map not found.");
    }
    const activeBoardCampaign = await runStoreQuery(async (database) => await readActiveBoardCampaign(database, campaignId), {
      queueKey: `campaign:${campaignId}`
    });
    broadcastMapUpsertToRoom(campaignId, map, {
        ...
        activeBoardCampaign && activeBoardCampaign.activeMapId === map.id
          ? (connection) =>
              connection.user
                ? {
                    mapId: map.id,
                    cells: connection.role === "dm" ? [] : normalizeExplorationMemoryForMap(activeBoardCampaign, connection.user.id, map.id)
                  }
                : undefined
          : undefined
    });
