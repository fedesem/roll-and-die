...
export const compendiumReferenceTablesMigration: Migration = {
  ...
  up(database) {
    if (tableExists(database, "compendium_spells")) {
      addColumnIfMissing(database, "compendium_spells", "higher_level_description", "TEXT NOT NULL DEFAULT ''");
    }

    if (tableExists(database, "compendium_classes")) {
      addColumnIfMissing(database, "compendium_classes", "hit_die_faces", "INTEGER NOT NULL DEFAULT 0");
      addColumnIfMissing(database, "compendium_classes", "primary_abilities_json", "TEXT NOT NULL DEFAULT '[]'");
      addColumnIfMissing(database, "compendium_classes", "saving_throw_proficiencies_json", "TEXT NOT NULL DEFAULT '[]'");
      addColumnIfMissing(database, "compendium_classes", "starting_armor_json", "TEXT NOT NULL DEFAULT '[]'");
      addColumnIfMissing(database, "compendium_classes", "starting_weapons_json", "TEXT NOT NULL DEFAULT '[]'");
      addColumnIfMissing(database, "compendium_classes", "starting_tools_json", "TEXT NOT NULL DEFAULT '[]'");
    }
    ...
    if (tableExists(database, "compendium_spells") && readCount(database, "compendium_spell_classes") === 0) {
      const spellRows = readAll<{ id: string; classesJson: string }>(
        database,
        `
          SELECT
            id,
    ...
    if (tableExists(database, "compendium_classes") && readCount(database, "compendium_class_features") === 0) {
      const classRows = readAll<{ id: string; source: string; featuresJson: string }>(
        database,
        `
          SELECT
            id,
    ...
    if (
      tableExists(database, "compendium_classes") &&
      readCount(database, "compendium_class_tables") === 0 &&
      readCount(database, "compendium_class_table_columns") === 0 &&
      readCount(database, "compendium_class_table_cells") === 0
    ) {
      const classRows = readAll<{ id: string; tablesJson: string }>(
        database,
        `
          SELECT
            id,
