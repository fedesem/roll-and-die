...
function parseTokenStatusMarkers(value: string | null | undefined): TokenStatusMarker[] {
    ...
    try {
        ...
        return Array.from(new Set(parsed.filter((entry): entry is TokenStatusMarker => typeof entry === "string" && tokenStatusMarkerSet.has(entry))));
    }
    catch {
        return [];
    }
...
export async function readRealtimeCampaign(database: DatabaseSync, campaignId: string): Promise<Campaign | null> {
    ...
    campaign.members = (await readAll<{
        userId: string;
        name: string;
        email: string;
    ...
    `, campaignId)).map((row) => createRealtimeActorShell(campaignId, row));
...
export async function readCampaignMembersAndInvites(database: DatabaseSync, campaignId: string) {
    return {
        ...
        WHERE campaign_id = ?
        ORDER BY sort_order, user_id
      `, campaignId)).map((member) => ({
            userId: member.userId,
            name: member.name,
            email: member.email,
            role: member.role
        })),
        invites: (await readAll<{
            id: string;
            code: string;
            label: string;
...
export async function readOwnedActorIdsForCampaignUser(database: DatabaseSync, campaignId: string, userId: string) {
    return (await readAll<{
      ...
      ORDER BY sort_order, id
    `, campaignId, userId)).map((row) => row.id);
}
export async function readChatActorContextById(database: DatabaseSync, campaignId: string, actorId: string): Promise<ChatActorContext | null> {
    const row = await database
        .prepare(`
...
export async function readActorById(database: DatabaseSync, campaignId: string, actorId: string): Promise<ActorSheet | null> {
    ...
    actor.spellSlots = (await readAll<{
        level: number;
        total: number;
        used: number;
    }>(database, `
    ...
    actor.armorItems = (await readAll<{
        id: string;
        name: string;
...
function writeCampaignRecord(statements: CampaignWriteStatements, campaign: Campaign, campaignOrder: number) {
    statements.insertCampaign.run(campaign.id, campaignOrder, campaign.name, campaign.createdAt, campaign.createdBy, campaign.activeMapId, JSON.stringify(campaign.allowedSourceBooks));
    ...
    campaign.actors.forEach((actor, actorOrder) => {
        ...
        actor.skills.forEach((skill, skillOrder) => {
            statements.insertActorSkill.run(actor.id, skill.id, skillOrder, skill.name, skill.ability, toIntegerBoolean(skill.proficient), toIntegerBoolean(skill.expertise));
        });
        ...
        actor.attacks.forEach((attack, attackOrder) => {
            statements.insertActorAttack.run(actor.id, attack.id, attackOrder, attack.name, attack.attackBonus, attack.damage, attack.damageType, attack.notes);
        });
        actor.armorItems.forEach((item, itemOrder) => {
            statements.insertActorArmorItem.run(actor.id, item.id, itemOrder, item.name, item.kind, item.armorClass, item.maxDexBonus, item.bonus, toIntegerBoolean(item.equipped), item.notes);
        });
        actor.bonuses.forEach((bonus, bonusOrder) => {
            statements.insertActorBonus.run(actor.id, bonus.id, bonusOrder, bonus.name, bonus.sourceType, bonus.targetType, bonus.targetKey, bonus.value, toIntegerBoolean(bonus.enabled));
        });
        actor.resources.forEach((resource, resourceOrder) => {
            statements.insertActorResource.run(actor.id, resource.id, resourceOrder, resource.name, resource.current, resource.max, resource.resetOn, resource.restoreAmount);
        });
        actor.inventory.forEach((item, itemOrder) => {
            statements.insertActorInventory.run(actor.id, item.id, itemOrder, item.name, item.type, item.quantity, toIntegerBoolean(item.equipped), item.notes);
        });
    });
    ...
    campaign.chat.forEach((message, messageOrder) => {
        ...
        statements.insertChatRoll.run(message.roll.id, message.id, message.roll.label, message.roll.notation, message.roll.modifier, message.roll.total, message.roll.breakdown ?? null, message.roll.createdAt);
...
async function readCampaignAggregateById(database: DatabaseSync, campaignId: string) {
    ...
    campaign.invites = (await readAll<{
        id: string;
        code: string;
        label: string;
        role: CampaignInvite["role"];
    ...
    `, campaignId)) {
        const message: ChatMessage = {
            ...
            createdAt: row.createdAt,
            kind: row.kind,
            actor: row.actorId && row.actorName && row.actorImageUrl !== null && row.actorColor
                ? {
                    actorId: row.actorId,
                    actorName: row.actorName,
                    actorImageUrl: row.actorImageUrl,
...
export async function insertCampaignMemberRecord(database: DatabaseSync, campaignId: string, member: CampaignMember) {
    database
        ...
        VALUES (?, ?, ?, ?, ?, ?)
      `)
        .run(campaignId, member.userId, await readNextSortOrder(database, "campaign_members", "campaign_id", campaignId), member.name, member.email, member.role);
}
export async function insertCampaignInviteRecord(database: DatabaseSync, campaignId: string, invite: CampaignInvite) {
    database
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)
      `)
        .run(invite.id, campaignId, await readNextSortOrder(database, "campaign_invites", "campaign_id", campaignId), invite.code, invite.label, invite.role, invite.createdAt, invite.createdBy);
}
...
export async function upsertActorRecord(database: DatabaseSync, campaignId: string, actor: ActorSheet) {
    const sortOrder = await readExistingSortOrder(database, "actors", actor.id) ?? await readNextSortOrder(database, "actors", "campaign_id", campaignId);
    database
        .prepare(`
        INSERT INTO actors (
          id, campaign_id, sort_order, owner_id, template_id, name, kind, image_url, class_name, species, background, alignment, level,
    ...
    actor.classes.forEach((actorClass, classOrder) => {
        database
          ...
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `)
            .run(actor.id, actorClass.id, classOrder, actorClass.compendiumId, actorClass.name, actorClass.source, actorClass.level, actorClass.hitDieFaces, actorClass.usedHitDice, actorClass.spellcastingAbility);
    });
    actor.skills.forEach((skill, skillOrder) => {
        database
            .prepare(`
          ...
          VALUES (?, ?, ?, ?, ?, ?, ?)
        `)
            .run(actor.id, skill.id, skillOrder, skill.name, skill.ability, toIntegerBoolean(skill.proficient), toIntegerBoolean(skill.expertise));
    });
    actor.spellSlots.forEach((slot) => {
        database
            .prepare(`
    ...
    actor.armorItems.forEach((item, itemOrder) => {
        database
          ...
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `)
            .run(actor.id, item.id, itemOrder, item.name, item.kind, item.armorClass, item.maxDexBonus, item.bonus, toIntegerBoolean(item.equipped), item.notes);
    });
    actor.bonuses.forEach((bonus, bonusOrder) => {
        database
            .prepare(`
          ...
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        `)
            .run(actor.id, bonus.id, bonusOrder, bonus.name, bonus.sourceType, bonus.targetType, bonus.targetKey, bonus.value, toIntegerBoolean(bonus.enabled));
    });
    actor.resources.forEach((resource, resourceOrder) => {
        database
            .prepare(`
...
export async function upsertMapRecord(database: DatabaseSync, campaignId: string, map: CampaignMap) {
    const sortOrder = await readExistingSortOrder(database, "maps", map.id) ?? await readNextSortOrder(database, "maps", "campaign_id", campaignId);
    database
        .prepare(`
        INSERT INTO maps (
          id, campaign_id, sort_order, name, background_url, background_offset_x, background_offset_y, background_scale,
    ...
    map.teleporters.forEach((teleporter, teleporterOrder) => {
        database
          ...
          VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        `)
            .run(teleporter.id, map.id, teleporterOrder, teleporter.pairNumber, teleporter.pointA.x, teleporter.pointA.y, teleporter.pointB.x, teleporter.pointB.y);
    });
...
export async function insertChatMessageRecord(database: DatabaseSync, message: ChatMessage) {
    ...
    database
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `)
        .run(message.id, message.campaignId, sortOrder, message.userId, message.userName, message.text, message.createdAt, message.kind, message.actor?.actorId ?? null, message.actor?.actorName ?? null, message.actor?.actorImageUrl ?? null, message.actor?.actorColor ?? null);
    if (!message.roll) {
        return;
    }
    database
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)
      `)
        .run(message.roll.id, message.id, message.roll.label, message.roll.notation, message.roll.modifier, message.roll.total, message.roll.breakdown ?? null, message.roll.createdAt);
    message.roll.rolls.forEach((value, valueOrder) => {
        database
            .prepare(`
          INSERT INTO chat_roll_values (roll_id, sort_order, value)
...
function insertActorTextEntries(database: DatabaseSync, actorId: string, kind: "features" | "spells" | "talents" | "feats", values: string[]) {
...
function writeActorTextEntries(statement: ReturnType<DatabaseSync["prepare"]>, actorId: string, kind: "features" | "spells" | "talents" | "feats", values: string[]) {
    values.forEach((value, index) => {
        statement.run(actorId, kind, index, value);
    });
}
function createRealtimeActorShell(campaignId: string, row: {
    id: string;
    ownerId: string | null;
    name: string;
    kind: ActorKind;
