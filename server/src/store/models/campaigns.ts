...
export function readCampaignCoreById(database: DatabaseSync, campaignId: string) {
  const row = database
    .prepare(
      `
        SELECT
...
export function readCampaignActiveMapId(database: DatabaseSync, campaignId: string) {
  const row = database
    .prepare(
      `
    ...
    .get(campaignId) as { activeMapId: string } | undefined;

  return row?.activeMapId ?? null;
}

export function readCampaignRoleForUser(database: DatabaseSync, campaignId: string, userId: string) {
  const row = database
    .prepare(
      `
        SELECT role
    ...
    .get(campaignId, userId) as { role: CampaignMember["role"] } | undefined;

  return row?.role ?? null;
}

export function readCampaignMembers(database: DatabaseSync, campaignId: string) {
  return readAll<{
    userId: string;
    name: string;
    email: string;
...
export function readCampaignSummaryForUser(database: DatabaseSync, campaignId: string, userId: string) {
  const row = database
    .prepare(
      `
        SELECT
...
export function readCampaignInviteByCodeRecord(database: DatabaseSync, code: string) {
  const row = database
    .prepare(
      `
        SELECT
...
export function readMapExists(database: DatabaseSync, campaignId: string, mapId: string) {
  const row = database
    .prepare(
      `
        SELECT 1 as found
    ...
    .get(campaignId, mapId) as { found: number } | undefined;

  return Boolean(row?.found);
}

export function readMapAssignment(database: DatabaseSync, campaignId: string, mapId: string, actorId: string) {
  const row = database
    .prepare(
      `
        SELECT map_id as mapId, actor_id as actorId
    ...
    .get(campaignId, mapId, actorId) as { mapId: string; actorId: string } | undefined;

  return row ? { mapId: row.mapId, actorId: row.actorId } : null;
}

export function readMapAssignmentsForActor(database: DatabaseSync, campaignId: string, actorId: string) {
  return readAll<{ mapId: string; actorId: string }>(
    database,
    `
      SELECT map_id as mapId, actor_id as actorId
      FROM map_actor_assignments
...
export function readTokenById(database: DatabaseSync, campaignId: string, tokenId: string): BoardToken | null {
  const row = database
    .prepare(
      `
        SELECT
...
export function readTokensForActor(database: DatabaseSync, campaignId: string, actorId: string) {
  return readAll<{
    id: string;
    actorId: string;
    actorKind: ActorKind;
...
export function readOwnedActorIdsForCampaignUser(database: DatabaseSync, campaignId: string, userId: string) {
  return readAll<{ id: string }>(
    database,
    `
      SELECT id
      FROM actors
...
export function readChatActorContextById(
  database: DatabaseSync,
  campaignId: string,
  actorId: string
): ChatActorContext | null {
  const row = database
    .prepare(
      `
...
export function readActorById(database: DatabaseSync, campaignId: string, actorId: string): ActorSheet | null {
  const row = database
    .prepare(
      `
        SELECT
  ...
  actor.spellSlots = readAll<{ level: number; total: number; used: number }>(
    database,
    `
  ...
  for (const entry of readAll<{ kind: "features" | "spells" | "talents" | "feats"; value: string }>(
    database,
    `
      SELECT kind, value
...
export function readCampaignBoardState(database: DatabaseSync, campaignId: string, mapIds: string[]) {
  const campaignCore = readCampaignCoreById(database, campaignId);
  ...
  const campaign: Campaign = {
    ...campaignCore,
    members: readCampaignMembers(database, campaignId),
    invites: [],
    actors: [],
    maps: [],
    mapAssignments: [],
...
export function listCampaignSummariesForUser(database: DatabaseSync, userId: string): CampaignSummary[] {
  return readAll<{
    id: string;
    name: string;
    createdAt: string;
...
export function listCampaignIdsForMember(database: DatabaseSync, userId: string) {
  return readAll<{ campaignId: string }>(
    database,
    `
...
function readCampaignAggregateById(database: DatabaseSync, campaignId: string) {
  const campaignRow = database
    .prepare(
      `
        SELECT
...
export function readCampaignSnapshotById(database: DatabaseSync, campaignId: string) {
  return readCampaignAggregateById(database, campaignId);
}

export function insertCampaignRecord(database: DatabaseSync, campaign: Campaign) {
  writeCampaign(database, campaign, readNextSortOrder(database, "campaigns"));
}
...
export function insertCampaignMemberRecord(
  database: DatabaseSync,
  campaignId: string,
  ...
  database
    ...
    .run(
      campaignId,
      member.userId,
      readNextSortOrder(database, "campaign_members", "campaign_id", campaignId),
      member.name,
      member.email,
      member.role
    );
...
export function insertCampaignInviteRecord(
  database: DatabaseSync,
  campaignId: string,
  ...
  database
    ...
    .run(
      invite.id,
      campaignId,
      readNextSortOrder(database, "campaign_invites", "campaign_id", campaignId),
      invite.code,
      invite.label,
      invite.role,
      invite.createdAt,
...
export function deleteCampaignInvitesByCreator(
  database: DatabaseSync,
  campaignId: string,
  createdBy: string
) {
...
export function updateCampaignMemberRole(
  database: DatabaseSync,
  campaignId: string,
  userId: string,
  role: CampaignMember["role"]
...
export function deleteCampaignChatMessagesByUser(
  database: DatabaseSync,
  campaignId: string,
  userId: string
) {
...
export function deleteCampaignExplorationForUser(
  database: DatabaseSync,
  campaignId: string,
  userId: string
) {
...
export function upsertActorRecord(database: DatabaseSync, campaignId: string, actor: ActorSheet) {
  const sortOrder =
    readExistingSortOrder(database, "actors", actor.id) ??
    readNextSortOrder(database, "actors", "campaign_id", campaignId);
...
export function upsertMapRecord(database: DatabaseSync, campaignId: string, map: CampaignMap) {
  const sortOrder =
    readExistingSortOrder(database, "maps", map.id) ??
    readNextSortOrder(database, "maps", "campaign_id", campaignId);
  ...
  map.drawings.forEach((drawing) => {
    insertDrawingRecord(database, map.id, drawing);
  });
}

export function insertMapAssignmentRecord(
  database: DatabaseSync,
  campaignId: string,
...
export function deleteMapAssignmentRecord(
  database: DatabaseSync,
  campaignId: string,
  mapId: string,
  actorId: string
...
export function replaceCampaignExploration(
  database: DatabaseSync,
  campaignId: string,
...
export function replaceMapExploration(
  database: DatabaseSync,
  campaignId: string,
  mapId: string,
  exploration: Campaign["exploration"]
...
export function replaceUserMapExploration(
  database: DatabaseSync,
  campaignId: string,
  userId: string,
  mapId: string,
...
export function insertChatMessageRecord(database: DatabaseSync, message: ChatMessage) {
  const sortOrder = readNextSortOrder(database, "chat_messages", "campaign_id", message.campaignId);
...
export function trimCampaignChatRecords(database: DatabaseSync, campaignId: string, limit = 200) {
  const overflow = readAll<{ id: string }>(
    database,
    `
      SELECT id
      FROM chat_messages
...
export function insertDrawingRecord(database: DatabaseSync, mapId: string, drawing: DrawingStroke) {
  const sortOrder = readNextSortOrder(database, "map_drawings", "map_id", mapId);
...
function readExistingSortOrder(database: DatabaseSync, table: string, id: string) {
  const row = database
    .prepare(`SELECT sort_order as sortOrder FROM ${table} WHERE id = ? LIMIT 1`)
    .get(id) as { sortOrder: number } | undefined;
...
function readNextSortOrder(
  ...
  if (filterColumn) {
    ...
    const row = database
      .prepare(
        `SELECT COALESCE(MAX(sort_order), -1) + 1 as nextSortOrder FROM ${table} WHERE ${filterColumn} = ?`
      )
      .get(filterValue) as { nextSortOrder: number };

    return row.nextSortOrder;
  }

  const row = database
    .prepare(`SELECT COALESCE(MAX(sort_order), -1) + 1 as nextSortOrder FROM ${table}`)
