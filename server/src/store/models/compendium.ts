...
export async function compendiumEntryExists(database: DatabaseSync, kind: CompendiumCollectionKind, entryId: string) {
  switch (kind) {
    ...
    case "races":
    case "skills":
      return Boolean(await database.prepare("SELECT 1 FROM compendium_references WHERE kind = ? AND id = ? LIMIT 1").get(kind, entryId));
  }
}
...
export async function insertCompendiumEntriesAtStart<K extends CompendiumCollectionKind>(
  database: DatabaseSync,
  kind: K,
...
export async function upsertCompendiumEntry<K extends CompendiumCollectionKind>(
  database: DatabaseSync,
  kind: K,
...
export function deleteCompendiumEntryRecord(database: DatabaseSync, kind: CompendiumCollectionKind, entryId: string) {
  switch (kind) {
    ...
    case "races":
    case "skills":
      database.prepare("DELETE FROM compendium_references WHERE kind = ? AND id = ?").run(kind, entryId);
      return;
  }
}
...
async function shiftCompendiumSortOrders(database: DatabaseSync, kind: CompendiumCollectionKind, amount: number) {
  ...
  switch (kind) {
    ...
    case "races":
    case "skills":
      await database.prepare("UPDATE compendium_references SET sort_order = sort_order + ? WHERE kind = ?").run(amount, kind);
      return;
  }
}
async function readExistingCompendiumSortOrder(database: DatabaseSync, kind: CompendiumCollectionKind, entryId: string) {
  switch (kind) {
    case "spells": {
      const row = await database.prepare("SELECT sort_order as sortOrder FROM compendium_spells WHERE id = ? LIMIT 1").get<{
        sortOrder: number;
      }>(entryId);
      return row?.sortOrder ?? null;
    }
    case "monsters": {
      const row = await database.prepare("SELECT sort_order as sortOrder FROM compendium_monsters WHERE id = ? LIMIT 1").get<{
        sortOrder: number;
      }>(entryId);
      return row?.sortOrder ?? null;
    }
    case "feats": {
      const row = await database.prepare("SELECT sort_order as sortOrder FROM compendium_feats WHERE id = ? LIMIT 1").get<{
        sortOrder: number;
      }>(entryId);
      return row?.sortOrder ?? null;
    }
    case "classes": {
      const row = await database.prepare("SELECT sort_order as sortOrder FROM compendium_classes WHERE id = ? LIMIT 1").get<{
        sortOrder: number;
      }>(entryId);
      return row?.sortOrder ?? null;
    }
    case "books": {
      const row = await database.prepare("SELECT sort_order as sortOrder FROM compendium_books WHERE source = ? LIMIT 1").get<{
        sortOrder: number;
      }>(entryId);
      return row?.sortOrder ?? null;
    }
...
async function readNextCompendiumSortOrder(database: DatabaseSync, kind: CompendiumCollectionKind) {
  switch (kind) {
    case "spells": {
      const row = await database.prepare("SELECT COALESCE(MAX(sort_order), -1) + 1 as nextSortOrder FROM compendium_spells").get<{
        nextSortOrder: number;
      }>();
      return row?.nextSortOrder ?? 0;
    }
    case "monsters": {
      const row = await database.prepare("SELECT COALESCE(MAX(sort_order), -1) + 1 as nextSortOrder FROM compendium_monsters").get<{
        nextSortOrder: number;
      }>();
      return row?.nextSortOrder ?? 0;
    }
    case "feats": {
      const row = await database.prepare("SELECT COALESCE(MAX(sort_order), -1) + 1 as nextSortOrder FROM compendium_feats").get<{
        nextSortOrder: number;
      }>();
      return row?.nextSortOrder ?? 0;
    }
    case "classes": {
      const row = await database.prepare("SELECT COALESCE(MAX(sort_order), -1) + 1 as nextSortOrder FROM compendium_classes").get<{
        nextSortOrder: number;
      }>();
      return row?.nextSortOrder ?? 0;
    }
    case "books": {
      const row = await database.prepare("SELECT COALESCE(MAX(sort_order), -1) + 1 as nextSortOrder FROM compendium_books").get<{
        nextSortOrder: number;
      }>();
      return row?.nextSortOrder ?? 0;
    }
