...
function buildPlayerVisionUpdateForMap(campaign: Parameters<typeof buildCampaignSnapshot>[0], userId: string, mapId: string): RoomPlayerVisionUpdate {
...
export function broadcastActorRemovedToRoom(campaignId: string, actorId: string, options?: {
...
export function broadcastMapUpsertToRoom(campaignId: string, map: CampaignMap, options?: {
    activeMapId?: string;
...
export function broadcastMapAssignmentsToRoom(campaignId: string, patch: {
...
export function broadcastTokenPatchToRoom(campaignId: string, patch: {
...
function persistExplorationDeltaForMap(campaignId: string, campaign: Parameters<typeof buildCampaignSnapshot>[0], mapId: string, previous: Map<string, string[]>, database: Parameters<typeof readActiveBoardCampaign>[0]) {
    for (const member of campaign.members) {
        if (member.role !== "player") {
            continue;
        }
...
function broadcastTokenMovedToRoom(campaignId: string, campaign: Parameters<typeof buildCampaignSnapshot>[0], token: BoardToken, mapId: string) {
    for (const connection of roomConnections) {
        if (!connection.user || connection.campaignId !== campaignId) {
            continue;
        }
    ...
    try {
        ...
        if (payload.type === "room:join") {
            const [{ session, userRecord, campaign }, compendium] = await Promise.all([
                await runStoreQuery(async (database) => {
        ...
        if (payload.type === "token:move") {
            const { campaign, token, activeMapId } = await runStoreTransaction(async (database) => {
        ...
        if (payload.type === "drawing:create") {
            ...
            const map = await runStoreQuery(async (database) => await readMapEditorMap(database, campaignId, payload.mapId), { queueKey: `campaign:${campaignId}` });
            if (map) {
