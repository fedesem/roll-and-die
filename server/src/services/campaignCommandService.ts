...
export async function createInviteCommand(params: { campaignId: string; userId: string; label?: string; role: CampaignInvite["role"] }) {
...
export async function deleteInviteCommand(params: { campaignId: string; userId: string; inviteId: string }) {
  return await runCampaignTransaction(params.campaignId, async (database) => {
    requireDungeonMasterForCampaign(database, params.campaignId, params.userId);
...
export async function updateActorCommand(params: { campaignId: string; actorId: string; userId: string; patch: unknown }) {
  const patch =
  ...
  const result = await runCampaignTransaction(params.campaignId, async (database) => {
    ...
    await upsertActorRecord(database, params.campaignId, actor);
    const affectedMapIds = Array.from(
      new Set((await readTokensForActor(database, params.campaignId, actor.id)).map((entry) => entry.mapId))
    );
...
export async function createMonsterActorCommand(params: { campaignId: string; userId: string; templateId: string; mapId?: string }) {
...
export async function deleteActorCommand(params: { campaignId: string; actorId: string; userId: string }) {
  return await runCampaignTransaction(params.campaignId, async (database) => {
    const role = await requireCampaignMemberRole(database, params.campaignId, params.userId);
    const actor = await readActorById(database, params.campaignId, params.actorId);
    if (!actor) {
...
export async function createMapCommand(params: { campaignId: string; userId: string; body: unknown }) {
  const body =
...
export async function updateMapCommand(params: { campaignId: string; mapId: string; userId: string; patch: unknown }) {
  const patch =
    params.patch && typeof params.patch === "object"
      ? {
          ...(params.patch as Record<string, unknown>),
...
export async function assignActorToMapCommand(params: { campaignId: string; mapId: string; actorId: string; userId: string }) {
  return await runCampaignTransaction(params.campaignId, async (database) => {
    const role = await requireCampaignMemberRole(database, params.campaignId, params.userId);
...
export async function removeActorFromMapCommand(params: { campaignId: string; mapId: string; actorId: string; userId: string }) {
  return await runCampaignTransaction(params.campaignId, async (database) => {
    const role = await requireCampaignMemberRole(database, params.campaignId, params.userId);
    const campaignCore = await requireCampaignCore(database, params.campaignId);
    const actor = await readActorById(database, params.campaignId, params.actorId);
    ...
    const removedTokens = (await readTokensForActor(database, params.campaignId, params.actorId)).filter(
...
export async function updateTokenCommand(params: {
  ...
  return await runCampaignTransaction(params.campaignId, async (database) => {
    ...
    const nextRotationDegrees =
      actor.kind === "static" ? normalizeTokenRotation(params.patch.rotationDegrees ?? storedToken.rotationDegrees) : 0;
...
export async function removeTokenCommand(params: { campaignId: string; tokenId: string; userId: string }) {
  return await runCampaignTransaction(params.campaignId, async (database) => {
    requireDungeonMasterForCampaign(database, params.campaignId, params.userId);
    const token = await readTokenById(database, params.campaignId, params.tokenId);
    if (!token) {
...
export async function createDrawingCommand(params: { campaignId: string; userId: string; mapId: string; stroke: DrawingStroke }) {
  return await runCampaignTransaction(params.campaignId, async (database) => {
...
export async function deleteDrawingsCommand(params: { campaignId: string; userId: string; mapId: string; drawingIds: string[] }) {
  return await runCampaignTransaction(params.campaignId, async (database) => {
    const role = await requireCampaignMemberRole(database, params.campaignId, params.userId);
    const activeMapId = (await requireCampaignCore(database, params.campaignId)).activeMapId;
    const activeMap = await readMapEditorMap(database, params.campaignId, activeMapId);
...
export async function clearDrawingsCommand(params: { campaignId: string; userId: string; mapId: string }) {
  return await runCampaignTransaction(params.campaignId, async (database) => {
    requireDungeonMasterForCampaign(database, params.campaignId, params.userId);
    const activeMapId = (await requireCampaignCore(database, params.campaignId)).activeMapId;
    if (params.mapId !== activeMapId) {
...
export async function setActiveMapCommand(params: { campaignId: string; userId: string; mapId: string }) {
  return await runCampaignTransaction(params.campaignId, async (database) => {
    requireDungeonMasterForCampaign(database, params.campaignId, params.userId);
    if (!(await readMapExists(database, params.campaignId, params.mapId))) {
      throw new HttpError(404, "Map not found.");
...
export async function resetFogCommand(params: { campaignId: string; userId: string; mapId: string }) {
  return await runCampaignTransaction(params.campaignId, async (database) => {
    requireDungeonMasterForCampaign(database, params.campaignId, params.userId);
    if (!(await readMapExists(database, params.campaignId, params.mapId))) {
      throw new HttpError(404, "Map not found.");
...
export async function clearFogCommand(params: { campaignId: string; userId: string; mapId: string }) {
  return await runCampaignTransaction(params.campaignId, async (database) => {
    requireDungeonMasterForCampaign(database, params.campaignId, params.userId);
    const activeMapId = (await requireCampaignCore(database, params.campaignId)).activeMapId;
    if (params.mapId !== activeMapId) {
