...
export function AuthPage({
  ...
  return (
    <div className="grid min-h-[calc(100dvh-4.75rem)] gap-6 bg-[radial-gradient(circle_at_top_left,rgba(245,198,92,0.12),transparent_24%),linear-gradient(180deg,#090b0f,#11141d)] px-4 py-6 lg:grid-cols-[minmax(0,1.15fr)_minmax(24rem,30rem)] lg:px-8">
      <section className="rounded-[1.75rem] border border-amber-200/10 bg-white/[0.04] p-8 shadow-[0_32px_90px_rgba(0,0,0,0.28)] backdrop-blur-xl">
      ...
      <section className="rounded-[1.75rem] border border-amber-200/12 bg-slate-950/75 p-6 shadow-[0_22px_70px_rgba(0,0,0,0.34)] backdrop-blur-xl">
        <div className="inline-flex rounded-full border border-amber-200/12 bg-white/[0.04] p-1">
          <button
            className={`rounded-full px-4 py-2 text-sm font-medium transition ${authMode === "login" ? "bg-amber-300/18 text-amber-50" : "text-slate-300 hover:text-amber-100"}`}
            type="button"
          ...
          <button
            className={`rounded-full px-4 py-2 text-sm font-medium transition ${authMode === "register" ? "bg-amber-300/18 text-amber-50" : "text-slate-300 hover:text-amber-100"}`}
            type="button"
        ...
        <form className="mt-6 grid gap-4" onSubmit={onSubmit}>
          {authMode === "register" && (
            <label className="grid gap-2 text-sm text-slate-300">
              Name
              <input className="h-12 rounded-2xl border border-white/10 bg-white/[0.04] px-4 text-slate-100 outline-none transition focus:border-amber-200/30 focus:bg-white/[0.06]" value={authForm.name} onChange={(event) => onAuthFormChange("name", event.target.value)} required />
            </label>
          )}
          <label className="grid gap-2 text-sm text-slate-300">
            Email
            <input className="h-12 rounded-2xl border border-white/10 bg-white/[0.04] px-4 text-slate-100 outline-none transition focus:border-amber-200/30 focus:bg-white/[0.06]" type="email" value={authForm.email} onChange={(event) => onAuthFormChange("email", event.target.value)} required />
          </label>
          <label className="grid gap-2 text-sm text-slate-300">
            Password
            <input
              className="h-12 rounded-2xl border border-white/10 bg-white/[0.04] px-4 text-slate-100 outline-none transition focus:border-amber-200/30 focus:bg-white/[0.06]"
              type="password"
          ...
          <button className="inline-flex h-12 items-center justify-center rounded-2xl border border-amber-200/20 bg-amber-300/18 px-5 text-sm font-semibold text-amber-50 transition hover:bg-amber-300/24" type="submit">
