...
export function CampaignJoinPage({ joinCode, hasInviteLink, onJoinCodeChange, onAcceptInvite, onBack }: CampaignJoinPageProps) {
  return (
    <main className="grid gap-5 px-4 py-6 lg:px-8">
      <section className="rounded-none border border-amber-200/10 bg-slate-950/72 p-6 shadow-[0_20px_70px_rgba(0,0,0,0.24)] backdrop-blur-xl">
        ...
        <p className="mt-3 max-w-2xl text-sm leading-6 text-slate-400">
          {hasInviteLink
            ? "This invite link already contains the campaign code. Review it below and join the room."
            : "Paste an invite code or open a shared invite link to join a campaign."}
        </p>
