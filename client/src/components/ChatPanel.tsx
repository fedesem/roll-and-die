    ...
    <section className="flex h-full min-h-0 flex-col rounded-none border border-white/10 bg-slate-950/78 shadow-[0_20px_60px_rgba(0,0,0,0.28)] backdrop-blur-xl">
      <div className="min-h-0 flex-1 overflow-y-auto px-3 py-3">
        {messages.map((message) => (
          <article
            key={message.id}
            className={`mb-2 rounded-none border px-3 py-2.5 text-sm ${
              message.kind === "roll"
          ...
          >
            ...
            {message.roll && (
              <div className="mt-2 inline-flex flex-wrap items-center gap-2 rounded-none border border-amber-200/12 bg-slate-950/55 px-3 py-1.5 text-xs text-amber-50">
