  ...
  override render() {
    ...
    return (
      <main className="flex min-h-screen items-center justify-center bg-[radial-gradient(circle_at_top,rgba(245,198,92,0.1),transparent_28%),linear-gradient(180deg,#090b0f,#10131b)] p-6">
        <section className="w-full max-w-2xl rounded-[1.6rem] border border-amber-200/12 bg-slate-950/82 p-8 shadow-[0_30px_90px_rgba(0,0,0,0.38)] backdrop-blur-xl">
          ...
          <button type="button" className="mt-5 inline-flex h-11 items-center justify-center rounded-2xl border border-amber-200/20 bg-amber-300/18 px-5 text-sm font-semibold text-amber-50 transition hover:bg-amber-300/24" onClick={() => window.location.reload()}>
            Reload
          </button>
          <pre className="mt-5 overflow-auto rounded-2xl border border-white/8 bg-white/[0.03] p-4 text-sm text-rose-200">{this.state.error.message}</pre>
        </section>
      </main>
    );
  }
