    ...
    <header className="sticky top-0 z-30 flex flex-wrap items-center justify-between gap-4 border-b border-amber-200/10 bg-slate-950/85 px-5 py-3 text-slate-100 backdrop-blur-xl">
      ...
      {showRoomStatus && campaignName && role && roomStatus && (
        <div className="flex min-w-0 flex-1 flex-wrap items-center justify-center gap-2">
          <span className="rounded-full border border-amber-300/20 bg-amber-300/12 px-3 py-1 text-xs font-semibold text-amber-100">{campaignName}</span>
          <span className="rounded-full border border-white/10 bg-white/6 px-3 py-1 text-xs text-slate-200">{activeMapName ?? "No map"}</span>
          <span className="rounded-full border border-white/10 bg-white/6 px-3 py-1 text-xs text-slate-200">{role.toUpperCase()}</span>
          <span
      ...
      <div className="flex items-center gap-2">
        ...
        <button
          type="button"
          className="rounded-full border border-white/12 bg-white/5 px-3 py-2 text-sm text-slate-200 transition hover:border-amber-200/18 hover:bg-white/8"
        ...
        <button
          type="button"
          className="rounded-full border border-white/12 bg-white/5 px-3 py-2 text-sm text-slate-200 transition hover:border-rose-200/18 hover:bg-white/8"
          onClick={onLogout}
        >
          Logout
        </button>
