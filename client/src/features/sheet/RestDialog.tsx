        ...
        <div className="flex items-start justify-between border-b border-amber-800/40 px-6 py-5">
          <div>
            ...
            <p className="mt-2 max-w-xl text-sm text-zinc-400">
              Choose how many hit dice to spend. Each die restores hit points and adds your Constitution modifier ({constitutionModifier >= 0 ? `+${constitutionModifier}` : constitutionModifier}).
            </p>
          </div>
          <button
            type="button"
        ...
        <div className="space-y-3 px-6 py-6">
          {classes.map((entry) => {
            ...
            return (
              <div
                key={entry.id}
                className="flex items-center justify-between border border-amber-800/40 bg-zinc-900/80 px-4 py-4"
              >
                <div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <span className="w-8 text-center text-lg font-semibold text-amber-50">
                    {currentValue}
                  </span>
                  <button
                    type="button"
